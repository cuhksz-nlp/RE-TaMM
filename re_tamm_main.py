...
from re_tamm_model import ReTamm
...
def train(args, model, tokenizer, processor, device, n_gpu, results={}):
    ...
    for epoch_num in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(train_iter):
            ...
            input_ids, input_mask, valid_ids, segment_ids, label_ids, e1_mask, e2_mask, \
            dep_key_list, dep_order_dep_type_matrix, dep_path_dep_type_matrix = batch

            loss = model(input_ids, segment_ids, input_mask, label_ids, e1_mask=e1_mask, e2_mask=e2_mask,
                         valid_ids=valid_ids, dep_key_list=dep_key_list,
                         dep_order_dep_adj_matrix=dep_order_dep_type_matrix, dep_order_dep_type_matrix=dep_order_dep_type_matrix,
                         dep_path_dep_adj_matrix=dep_path_dep_type_matrix, dep_path_dep_type_matrix=dep_path_dep_type_matrix)
            if n_gpu > 1:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask, e1_mask=e1_mask, e2_mask=e2_mask,
                           ...
                           dep_order_dep_adj_matrix=dep_order_dep_type_matrix,
                           dep_order_dep_type_matrix=dep_order_dep_type_matrix,
                           dep_path_dep_adj_matrix=dep_path_dep_type_matrix,
...
def get_args():
    ...
    parser.add_argument('--dep_order', type=str, default='first_order',choices=["first_order","second_order","third_order"])
...
def train_func(args):
    ...
    processor = RE_Processor(dep_order=args.dep_order)
    processor.prepare_keys_dict(args.data_dir)
    processor.prepare_vals_dict(args.data_dir)
    processor.prepare_labels_dict(args.data_dir)
    label_list = processor.labels_dict.keys()
    key_list = processor.keys_dict.keys()
    dep_label_list = processor.vals_dict.keys()
    num_labels = len(label_list)
    key_size = len(key_list)
    val_size = len(dep_label_list)
    ...
    config.__dict__["key_size"] = key_size
    config.__dict__["val_size"] = val_size
    config.__dict__["dep_order"] = args.dep_order
    # model = ReTamm(config)
